...
if (isset($_POST['files'])) {
    function getDataTable($draw, $start, $length, $search)
    {
        ...
        $query = "SELECT DATE_FORMAT(e.date_upload, '%m-%d-%Y %H:%i:%s') as date, e.file_name, e.id, c.bac_list 
        ...
        $totalQuery = "SELECT COUNT(*) as total FROM tbl_bac_upload WHERE 1=1";
        if (!empty($search)) {
